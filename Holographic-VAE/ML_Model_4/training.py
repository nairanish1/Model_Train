...
from ML_Model_4.dataloader import create_dataloader, Args
from ML_Model_4.model import ConvHyperCNN 
...
from ML_Model_4.metrics import rotationally_averaged_lsd, directional_grad_energy
...
def inverse_sh_transform(sh: torch.Tensor, Y: torch.Tensor) -> torch.Tensor:
    # Apply the inverspherical harmonic transformation.
    return torch.abs(sh @ Y.T)
...
def compute_avg_lsd(H_target: torch.Tensor, H_recon: torch.Tensor, eps: float = 1e-3) -> torch.Tensor:
    ...
    # log‑spectral distortion, then mean over all their elements
    lsd = 20.0 * torch.log10(ratio).abs()
    return lsd.mean()
...
def train_model(args, num_epochs, lr, patience,
    ...
    device = args.device
     ...
     # load the SH→Y basis for LSD/inversion
